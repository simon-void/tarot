...
public class TarotCardDeck {
  ...
  public TarotCardDeck(TarotConfiguration config) {
    pixelWidth = config.getCardPixelWidth();
  ...
  private void addMinorArcana(String arcanaName, TarotConfiguration config, List<TarotCardInfo> deckToAddCardsToo) {
    ...
    minorArcanaPathB.append(config.getMinorArcanaPath()).append("tarot-").append(arcanaName.toLowerCase())
        .append(".htm");
  ...
  public TarotCard[] pickRandomCards(final int howMany) {
    if (howMany > deck.size()) {
      throw new IllegalArgumentException("there are only " + getNumberOfCards() + " in the deck. so you can't pick "
          + howMany);
    }
    if (howMany < 0) {
      throw new IllegalArgumentException("you have to pick a positive amount of cards, not " + howMany);
    }
  ...
  private TarotCard getCardFromInfo(TarotCardInfo info) {
    TarotCard card = new TarotCard(info.name, info.explanationUrl, info.imageUrl, info.imageRotateUrl, pixelWidth);
  ...
  class DeckBuilder {
    ...
    public DeckBuilder(final String explainFolderPath, final String imgFolderPath,
        final List<TarotCardInfo> deckToAddCardsToo) {
    ...
    public void addCard(String name, String explanationFileName, String imageFileName) {
      TarotCardInfo info = new TarotCardInfo(name, explainFolderPath + explanationFileName, imgFolderPath
          + imageFileName, imgRotatedFolderPath + imageFileName);
